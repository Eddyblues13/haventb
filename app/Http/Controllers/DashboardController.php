...
class DashboardController extends Controller
{
    ...
    public function zelle()
    {
        ...
        $data['user_debit'] = Debit::where('user_id', Auth::user()->id)->where('status', '1')->sum('amount');
        $data['balance'] = $data['user_deposits'] + $data['credit_transfers'] + $data['user_loans'] - $data['debit_transfers'] - $data['user_card'];
        return view('dashboard.zelle', $data);
    }
    ...
    public function interTransfer(Request $request)
    {
        ...
        //     return back()->with('error', ' Incorrect  Reflection!');
        // }
        ...
        return back()->with('status', 'Withdrawal Pending....');
    }
    ...
    public function zelleTransfer(Request $request)
    {
        ...
        $transaction = new Transaction;
        $transaction->user_id = Auth::user()->id;
        $transaction->transaction_id = "ZEL" . $ref;
        $transaction->transaction_ref = "ZEL" . $ref;
        $transaction->transaction_type = "Debit";
        $transaction->transaction = "ZEL Withdrawal";
        $transaction->transaction_amount = $request['amount'];
        $transaction->transaction_description = "ZEL transaction";
        $transaction->transaction_status = 0;
        $transaction->save();
